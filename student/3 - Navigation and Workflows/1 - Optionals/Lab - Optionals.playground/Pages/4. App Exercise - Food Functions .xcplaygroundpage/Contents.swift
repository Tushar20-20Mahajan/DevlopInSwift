...
if let breakfastMeal = getMeal("Breakfast") {
    print("Breakfast Meal: \(breakfastMeal)")
} else {
    print("You haven't logged your breakfast yet.")
}

if let lunchMeal = getMeal("Lunch") {
    print("Lunch Meal: \(lunchMeal)")
} else {
    print("You haven't logged your lunch yet.")
}

if let dinnerMeal = getMeal("Dinner") {
    print("Dinner Meal: \(dinnerMeal)")
} else {
    print("You haven't logged your dinner yet.")
}
...
func retrieveMealLog() -> [String: Any] {
    if let savedMealLog = UserDefaults.standard.dictionary(forKey: "mealLog") {
        ...
        return [:] // Return an empty dictionary if no meal log is saved
    }
}
